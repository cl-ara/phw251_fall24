...
(b) round() requires `x` (the numeric vector) and `digits` (an integer representing the number of decimal places the vector will be rounded to)
...
```{r question_8}
ifelse(odds_ratio > 1, "greater than 1", "not greater than 1")
```
